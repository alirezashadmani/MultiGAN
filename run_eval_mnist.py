...
def cifar():
    ...
    output_dir = expanduser('~/output/multi_gan/mnist/final_mixed_nash/0')
...
def train(n_generators, n_discriminators, noise_dim, ngf, ndf, grad_penalty, sampling, mirror_lr, restart,
    ...
    for G, generator in players['G'].items():
        generator.eval()
        with torch.no_grad():
            fake_images[G] = ((generator(fixed_noise[:16]) + 1) / 2).numpy()
    joblib.dump(fake_images, 'fake_images_mnist.pkl')


def plot():
    fake_images = joblib.load('fake_images_mnist.pkl')
    print(len(fake_images))
    fig = plt.figure(figsize=(3, 2), constrained_layout=False, dpi=600)
    fig.subplots_adjust(left=0.03, right=0.97, bottom=0.2, top=0.9)
    print(fake_images)
    outer_grid = fig.add_gridspec(1, 2, wspace=.2, hspace=0.1)
    print(outer_grid)
    for i, this_grid in enumerate(outer_grid):
        if i == 5:
            ...
            inner_grid = this_grid.subgridspec(4, 4, wspace=0.0, hspace=0.0)
            for j, this_inner_grid in enumerate(inner_grid):
                ax = fig.add_subplot(this_inner_grid)
                # ax.plot(range(10), range(10))
                ax.axis('off')
                ax.imshow(fake_images[i][j][0], cmap='gray')
                fig.add_subplot(ax)
            ax = fig.add_subplot(this_grid)
            ax.axis('off')
            if 0 <= i <= 2:
            ...
            fig.add_subplot(ax)
            if i == 0:
                ax.annotate(f'MNIST images, WFR, 2 gen. 2 discr.', xy=(0.0, -0.2), xytext=(0, 0), xycoords='axes fraction',
                                ha='left', va='bottom',
                                textcoords='offset points', fontsize=12)
    plt.savefig('images_mnist.pdf')
    plt.show()
